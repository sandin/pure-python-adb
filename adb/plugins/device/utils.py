...
class Utils(Plugin):
    ...
    def get_meminfo(self, package_name):
        total_meminfo_re = re.compile(r"\s*TOTAL\s*(?P<pss>\d+)\s*(?P<private_dirty>\d+)\s*(?P<private_clean>\d+)\s*(?P<swapped_dirty>\d+)\s*(?P<heap_size>\d+)\s*(?P<heap_alloc>\d+)\s*(?P<heap_free>\d+)")
